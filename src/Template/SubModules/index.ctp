...
<div class="row">
		...
		<div class="col-md-6">
			<h3 class="box-title no-margin"><span class="glyphicon glyphicon-file"></span>Sub Modules</h3>
		</div>
...
<div class="col-md-12  text-right">
   <?php
                    if (array_key_exists("add", $module_pages)):
                        ...
                        foreach ($user_permission as $permission):
                            if ($permission['page_id'] == $page_id):?>
                                    ...
                                    <a href="<?php echo $this->Url->build('/submodules/add/'); ?>"  class="btn btn-success btn-flat btn-grid">
                                        <i class="fa fa-plus-square"></i>
                                        <?php echo __('ADD_SUB_MODULE'); ?>
                                    </a>
                    ...
                    ?>
<div class="btn-group">
                  <button type="button" class="btn btn-primary"><?php echo __('SUB_MODULE_MANAGEMENT'); ?></button>
                            ...
                            <?php
									...
									$page_id = $module_pages ["delete"];
										foreach ( $user_permission as $permission ) :
											if ($permission ['page_id'] == $page_id) :
										?>
                                        <?= $this->Form->create(null, ['url' => ['controller' => 'submodules', 'action' => 'bulkAction']]); ?>
				...
				<div class="col-md-6">
                       ...
                       <div class="box-tools">
                           ...
                           <div class="input-group-btn">
                                    <?=$this->Form->submit ( __ ( 'SEARCH_SUB_MODULE' ), [ 'class' => 'btn btn-success btn-flat' ] );?>
                                </div>
									</div>
								</div>
                        </div>
            ...
            <div class="box-body table-responsive ">
              <table class="table table-striped table-bordered table-hover">
                                ...
                                <tr>
                                    ...
                                    <th class="sorting_asc"><?= $this->Paginator->sort('SubModules.id', ['value' => __('SUB_MODULE_ID')]) ?></th>
                                    <th><?= $this->Paginator->sort('Modules.name', ['value' => __('MODULE_NAME')]) ?></th>
                                    <th><?= $this->Paginator->sort('SubModules.name', ['value' => __('SUB_MODULE_NAME')]) ?></th>
                                    <th><?= $this->Paginator->sort('SubModules.created_on', ['value' => __('CREATED_ON')]) ?></th>
                                    <th><?= $this->Paginator->sort('SubModules.modified_on', ['value' => __('MODIFIED_ON')]) ?></th>
                                ...
                                foreach ($subModules as $subModule): ?>
                                    <tr>
                                        <td>
                                            <div> <!-- <div class="checkbox"> -->
                                                <input type="checkbox" 
                                                       id="content_<?= $subModule->id ?>">
                                                <label class="image-checkbox-label"
                                                       for="content_<?= $subModule->id ?>"></label>
                                            </div>
                                        </td>
                                        ...
                                        <td><?= $subModule->created_on ?></td>
                                        <td><?= $subModule->modified_on ?></td>
                                        <td class="text-right">
                    ...
                    <li>
                     <?php
                                                    if (array_key_exists("edit", $module_pages)):
                                                        ...
                                                        foreach ($user_permission as $permission):
                                                            if ($permission['page_id'] == $page_id):?>
                                                                <a href="   <?php echo $this->Url->build(['action' => 'edit', $subModule->id]) ?>"><i class="fa fa-pencil"></i><?php echo __('EDIT') ?>
                                                                </a>
                                                                <?php
                                                                break;
                                                            endif;
                                                    ...
                                                    if (array_key_exists("delete", $module_pages)):
                                                        ...
                                                        foreach ($user_permission as $permission):
                                                            if ($permission['page_id'] == $page_id):?>
                                                                        ...
                                                                        <?php echo $this->Form->postLink(' <i class="fa fa-trash-o"></i>'.__('DELETE'), ['action' => 'delete', $subModule->id], ['confirm' => __('Are you sure you want to delete {0}?', $subModule->name),'escape'=>false]) ?>
...
<script>
    ...
    $(document).ready(function () {
        $(document).on('click', '.edit', function () {
            ...
            window.location = '<?php echo $this->Url->build('/submodules/edit/');?>' + cid;
            return false;
        });
